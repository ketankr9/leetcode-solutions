...
class Solution {
public:
	int callme(int headID, vector<vector<int>>& adj, vector<int>& informTime){
		if(adj[headID].empty())	return 0;
		int ret = 0;
		for(int subOrd: adj[headID])
			ret = max(ret, callme(subOrd, adj, informTime));
		return ret+informTime[headID];
	}
    ...
    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
        ...
        return callme(headID, adj, informTime);
    }
};
// @lc code=end
