...
public:
    bool PredictTheWinner(vector<int>& nums) {
        ...
        int dp[n][n][2];
        for(int i=0; i<n; i++)
            dp[i][i][0] = dp[i][i][1] = nums[i], total += nums[i];
        
        for(int k=1; k<n; k++)
            for(int i=0; i+k<n; i++){
                dp[i][i+k][1] = max(nums[i] + dp[i+1][i+k][0], nums[i+k] + dp[i][i+k-1][0]);
                dp[i][i+k][0] = min( dp[i+1][i+k][1], dp[i][i+k-1][1]);
            }
        return dp[0][n-1][1]*2 >= total;
    }
};
