...
public:
    ...
    unordered_map<int, vector<pr>> tweets;
    int timestamp = 0;
    Twitter() {
        f.clear();
    }
    ...
    struct comp{
        bool operator()(tp& a, tp& b){
            return get<0>(a) < get<0>(b); // lowest time stamp on top
        }
    };
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        for(auto const& user: f[userId]){
            if(tweets[user].empty())    continue;
            pq.push({tweets[user].back().f, user, (int)tweets[user].size()-1});
        }
        vector<int> ret;
        int k = 10;
        while(k-- && !pq.empty()){
            tp top = pq.top(); pq.pop();
            int user = get<1>(top), idx = get<2>(top);
            ret.push_back(tweets[user][idx].s);
            if(idx-->0)
                pq.push({tweets[user][idx].f, user, idx});
        }
        return ret;
    }
