...
public:
    int mincostTickets(vector<int>& days, vector<int>& costs) {
        ...
        for(int i=days.front(); i<=days.back(); i++){
            if(ss.find(i)==ss.end())
                dp[i%30] = (i-1<days.front()?0:dp[(i-1+30)%30]);
            else
                dp[i%30] = min( (i-1<days.front()?0:dp[(i-1+30)%30])+costs[0] , min( (i-7<days.front()?0:dp[(i-7+30)%30])+costs[1], (i-30<days.front()?0:dp[(i-30+30)%30])+costs[2]));
        }
