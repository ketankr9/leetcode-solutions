...
public:
    int openLock(vector<string>& deadends, string target) {
        ...
        while(!q.empty()){
            ...
            while(!q.empty()){
                for(int i=0; i<4; i++){
                    string child1 = q.front(); child1[i] = (child1[i]=='9'?'0':child1[i]+1);
                    string child2 = q.front(); child2[i] = (child2[i]=='0'?'9':child2[i]-1);
                    if(child1 == target || child2 == target)
                        return ans;
                    if(visited.count(child1)==0 && ss.count(child1)==0)
                        tmp.push(child1), visited.insert(child1);
                    if(visited.count(child2)==0 && ss.count(child2)==0)
                        tmp.push(child2), visited.insert(child2);
                }
                q.pop();
            }
            q = tmp;
