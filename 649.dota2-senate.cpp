...
public:
    string predictPartyVictory(string senate) {
        ...
        // int toskipR = 0, toskipC = 0;
        int deadR = 0, deadD = 0;
        while(true){
            for(int i=0; i<(int)senate.size(); i++){
                if(senate[i] == 'R'){
                    if(deadR>0){ deadR--; senate[i] = '#'; continue; }
                    deadD++;
                    totalD--;
                }else if(senate[i] == 'D'){ // == 'D'
                    if(deadD>0){ deadD--; senate[i] = '#'; continue; }
                    deadR++;
                    totalR--;
                }
            }
            if(totalD > 0 && totalR <= 0)  return "Dire";
            else if(totalR >0 && totalD <= 0) return "Radiant";   
        }
        return "DIRE";
