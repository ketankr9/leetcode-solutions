...
class Solution {
public:
	pair<int, int> callme(TreeNode* root){
		if(root == NULL)	return make_pair(0, 0);

		pair<int, int> l = callme(root->left);
		pair<int, int> r = callme(root->right);

		return make_pair(root->val + l.second + r.second, max(l.first, l.second) + max(r.first, r.second));
	}
    int rob(TreeNode* root) {
        pair<int, int> p = callme(root);
