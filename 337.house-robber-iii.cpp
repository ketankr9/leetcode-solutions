...
public:
	...
	int callme(TreeNode* root){
		...
		return mm[root] = max(
			root->val + 
			(root->left!=NULL?(callme(root->left->left) + callme(root->left->right)):0) + 
			(root->right!=NULL?(callme(root->right->left) + callme(root->right->right)):0),
			callme(root->left) + callme(root->right)
			);
	}
