...
class Solution {
private:
	long long next(string& st, int& i){
		long long ret = 0;
...
public:
    int compareVersion(string version1, string version2) {
       	int xi = 0, yi = 0;
        long long x = next(version1, xi);
        long long y = next(version2, yi);
        
        while(x == y && xi<version1.size() && yi<version2.size())
        		x = next(version1, xi), y = next(version2, yi);

    	if(x!=y || (xi<version1.size() && yi<version2.size()) || (xi == version1.size() && yi == version2.size())){
    		if(x > y)
    			return 1;
    		else if(x < y)
    			return -1;
    	...
    	x = 0ll, y = 0ll;
    	if(xi == version1.size()){ 
    		while(yi<version2.size())
    			y = max(y, next(version2, yi));
    		if( y > 0)
    			return -1;
    		return 0;

    	}else{  //yi == version2.size()
    		while(xi<version1.size())
    			x = max(x, next(version1, xi));    		
    		if( x > 0)
    			return 1;
    		return 0;
    	}
